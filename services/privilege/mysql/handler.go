...
func processPrivilegeSessionResults(jobID string, contextData interface{}, statusResp *job.StatusResponse, jobInfo *job.JobInfo) error {
	...
	sessionContext, ok := contextData.(*privilege.PrivilegeSessionJobContext)
	if !ok {
...
func processPrivilegeSessionFromNotification(jobID string, sessionContext *privilege.PrivilegeSessionJobContext, notificationData interface{}, statusResp *job.StatusResponse) error {
...
func processPrivilegeSessionFromVeloArtifact(jobID string, sessionContext *privilege.PrivilegeSessionJobContext, statusResp *job.StatusResponse) error {
...
func parsePrivilegeDataFile(filePath string) ([]privilege.QueryResult, error) {
...
func retrievePrivilegeDataResults(jobID string, ep *models.Endpoint) ([]privilege.QueryResult, error) {
...
func createPoliciesWithPrivilegeData(jobID string, sessionContext *privilege.PrivilegeSessionJobContext, privilegeData []privilege.QueryResult) (int, error) {
	...
	classification := classifyPolicyTemplates(service.GetPolicyDefaultsMap())
	...
	if len(classification.objectSpecificPrivs) > 0 {
		...
		allObjectQueries := make(map[string]privilege.PolicyInput)
...
func executeSuperPrivilegeQueries(
	tx *gorm.DB,
	session *privilege.PrivilegeSession,
	queries map[string]privilege.PolicyInput,
	...
	type queryResult struct {
		uniqueKey   string
		policyData  privilege.PolicyInput
		resultValue string
		err         error
	}
	...
	for uniqueKey, policyData := range queries {
		queryCount++
		go func(key string, input privilege.PolicyInput) {
			defer func() {
				if r := recover(); r != nil {
...
func executeActionWideQueries(
	...
	filteredQueries := make(map[string]privilege.PolicyInput)
...
func loadPrivilegeDataFromResults(session *privilege.PrivilegeSession, results []privilege.QueryResult) error {
...
func insertResultsIntoSession(session *privilege.PrivilegeSession, tableName string, rows [][]interface{}) error {
	if len(rows) == 0 {
...
func processQueriesWithSession(tx *gorm.DB, session *privilege.PrivilegeSession, sqlFinalMap map[string]privilege.PolicyInput, service privilege.PolicyEvaluator, dbmgt *models.DBMgt, cmt *models.CntMgt) int {
	type queryResult struct {
		uniqueKey   string
		policyInput privilege.PolicyInput
		resultValue string
		err         error
	}
...
func buildQueryBuildCache(tx *gorm.DB, cntID uint, allDatabases []models.DBMgt, policyDefaults []models.DBPolicyDefault, service privilege.PolicyEvaluator) (*queryBuildCache, error) {
	cache := &queryBuildCache{
		allActorsByCntID: make(map[uint][]*models.DBActorMgt),
		objectsByKey:     make(map[string][]*models.DBObjectMgt),
	}
	...
	for key := range objectKeysToLoad {
		...
		objects, err := service.GetDBObjectsByObjectIdAndDbMgt(tx, objectID, dbMgtID)
...
func processGeneralSQLTemplatesForSession(
	...
	cmt *models.CntMgt,
	service privilege.PolicyEvaluator,
) (map[string]privilege.PolicyInput, error) {
	sqlFinalMap := make(map[string]privilege.PolicyInput)
		...
		for _, actor := range actors {
			if hasDatabaseVar {
				for _, dbmgt := range allDatabases {
					...
					sqlFinalMap[uniqueKey] = privilege.PolicyInput{
						Policydf: policydf,
						ActorId:  actor.ID,
						ObjectId: -1,
